const agregarTarea = async () => {
    ...
    const response = await fetch('http://localhost:3000/tareas', {
        ...
        body: JSON.stringify({ descripcion: textValue }),
    });
    const tarea = await response.json();
    changeText(tarea.id, textValue);
};

function changeText(newId, textValue, active = 0) {
    if (active === 0) {
        active = "active"
    } else {
        active = "deactive"
...
function deleteListElement(elementToRemove) {
    ...
    fetch('http://localhost:3000/tareas/' + elementToRemove, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
    });
...
function changeClass(idOfElement) {
    ...
    if (classes[1] === "active"){
        fetch('http://localhost:3000/tareas/' + idOfElement.substring(0, idOfElement.length - 4), {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
...
const obtenerTareas = async () => {
    ...
    data.tareas.forEach((tarea) => changeText(tarea.id, tarea.descripcion, tarea.completada));
};
